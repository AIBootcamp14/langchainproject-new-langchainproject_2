...
def register_korean_font():
    ...
    try:
        ...
        if not fonts_loaded:
            logger.warning("backend/font 디렉터리에서 폰트를 찾을 수 없습니다. 시스템 기본 경로를 시도합니다.")
...
def create_trend_report_pdf(keyword: str, trend_data: Dict[str, Any], analysis: Dict[str, Any]) -> str:
    ...
    if peak:
        metrics_data.append(["검색 피크", f"{peak.get('date', 'N/A')} / {_format_metric(peak.get('value'), integer=True)}"])
    ...
    try:
        ...
        logger.error("트렌드 리포트 PDF 생성 실패: %s", e, exc_info=True)
        raise
